...
**Primary Variant**: `DupeRangerAi.py` is the canonical main developed version with full AI integration. Other variants (`AiDupeRanger_grok.py`, `AiDupeRanger_claude.py`, `AiDupeRanger.py`) exist for different configurations or legacy compatibility but are not the canonical build target.
