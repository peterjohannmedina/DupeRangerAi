...
class FileRecord:
    ...
    duplicate_primary: Path | None = None
...
@dataclass
class FileOperation:
    """Represents a planned or completed file operation."""
    operation_type: str  # "rename_duplicate", "organize_category"
    source_path: Path
...
class FileOperationManager:
    """Manages file renaming and organization operations."""

    def __init__(self, results: ScanResults, options: dict):
        self.results = results
        self.options = options
        self.operations: list[FileOperation] = []
        self.log_path = Path(options.get("log_path", "duperanger_operations.json"))

    def plan_duplicate_operations(self) -> list[FileOperation]:
        """
        Generate operations for marking duplicates.
        Returns list of FileOperation objects.
        """
        operations = []

        for hash_value, records in self.results.duplicates.items():
            if len(records) < 2:
                continue
            ...
            for record in records:
                if record.path == primary.path:
                    continue

                # Skip if already marked
                if record.path.name.startswith("._dr_"):
                    continue

                # Generate safe target name
                target = self._generate_duplicate_name(record.path)

                op = FileOperation(
                    operation_type="rename_duplicate",
    ...
    def plan_category_operations(self) -> list[FileOperation]:
        """
        Generate operations for organizing files by category.
        Returns list of FileOperation objects.
        """
        operations = []
        base_dir = Path(self.options["organize_base_dir"])

        for record in self.results.files:
            ...
            target = self._generate_category_path(record, base_dir)
            ...
            op = FileOperation(
                operation_type="organize_category",
                source_path=record.path,
                destination_path=target,
                file_record=record,
            )
    ...
    def execute_operations(
        ...
        for i, op in enumerate(operations):
            ...
            if not op.source_path.exists():
                op.status = "skipped"
                op.error_message = "Source file no longer exists"
                skipped += 1
                continue

            if op.destination_path.exists():
                op.status = "skipped"
                op.error_message = "Destination already exists"
                skipped += 1
                continue

            # Attempt operation
            try:
                if op.operation_type == "rename_duplicate":
                    op.source_path.rename(op.destination_path)
                elif op.operation_type == "organize_category":
                ...
                op.status = "failed"
                op.error_message = f"Error: {e}"
                failed += 1

            # Save to log after each operation
            if self.options.get("create_log", False):
                self._append_to_log(op)

        return success, failed, skipped

    def _select_primary(self, records: list[FileRecord]) -> FileRecord:
        """Select which file should be kept as primary."""
        if self.options["primary_mode"] == "oldest":
            # Use creation time
            return min(records, key=lambda r: r.creation_time or float('inf'))
        else:  # "last_modified"
            return max(records, key=lambda r: r.modification_time or 0)

    def _generate_duplicate_name(self, path: Path) -> Path:
        """Generate safe duplicate name with collision avoidance."""
        parent = path.parent
        base_name = f"._dr_{path.name}"
        ...
        counter = 1
        while True:
            candidate = parent / f"._dr_{stem}_{counter:02d}{suffix}"
            if not candidate.exists():
                return candidate
            counter += 1
            if counter > 9999:
    ...
    def _generate_category_path(self, record: FileRecord, base_dir: Path) -> Path:
        """Generate target path for category organization."""
        category_dir = base_dir / record.category
        target = category_dir / record.path.name
        ...
        counter = 1
        while True:
            target = category_dir / f"{stem}_{counter:02d}{suffix}"
            if not target.exists():
                return target
            counter += 1

    def _append_to_log(self, op: FileOperation):
...
class OperationPreviewDialog:
    """Modal dialog for previewing and confirming file operations."""

    def __init__(self, parent: tk.Tk, operations: list[FileOperation]):
        self.result = None  # Will be "apply" or "cancel"

        self.dialog = tk.Toplevel(parent)
        self.dialog.title("Preview File Operations")
        ...
        rename_count = sum(1 for op in operations if op.operation_type == "rename_duplicate")
        organize_count = sum(1 for op in operations if op.operation_type == "organize_category")

        summary_text = f"Total operations: {len(operations)}\n"
        if rename_count > 0:
            summary_text += f"  • Duplicate renaming: {rename_count} files\n"
        if organize_count > 0:
            summary_text += f"  • Category organization: {organize_count} files\n"
        ...
        # Create tree
        columns = ("operation", "source", "destination")
        ...
        # Populate tree
        for i, op in enumerate(operations):
            op_label = "Rename Duplicate" if op.operation_type == "rename_duplicate" else "Organize by Category"
            self.tree.insert(
                "", tk.END,
                text=str(i + 1),
                values=(op_label, str(op.source_path), str(op.destination_path))
            )
        ...
        ttk.Button(button_frame, text="Apply Operations", command=self._on_apply).pack(side=tk.RIGHT, padx=(5, 0))
...
class FileOrganizerApp:
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.root.title("ZFS File Organizer Helper - Enhanced by Claude")
        self.root.geometry("960x800")  # Increased height for new controls
        ...
        self.operation_manager: FileOperationManager | None = None
    ...
    def _build_ui(self) -> None:
        ...
        # NEW: Duplicate handling section
        dup_frame = ttk.LabelFrame(options_frame, text="Duplicate Handling", padding=10)
        dup_frame.grid(column=0, row=7, columnspan=2, sticky="ew", pady=(10, 0))

        self.handle_duplicates_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(
            dup_frame,
            text="Mark duplicate files with ._dr_ prefix",
            variable=self.handle_duplicates_var,
        ).grid(column=0, row=0, sticky="w")

        ttk.Label(dup_frame, text="Keep as primary:").grid(column=0, row=1, sticky="w", pady=(6, 0))
        self.primary_selection_var = tk.StringVar(value="oldest")
        ttk.Radiobutton(
            dup_frame,
            text="Oldest file (by creation date)",
            variable=self.primary_selection_var,
            value="oldest"
        ).grid(column=0, row=2, sticky="w", padx=(20, 0))
        ttk.Radiobutton(
            dup_frame,
            text="Last modified (most recent edit)",
            variable=self.primary_selection_var,
            value="last_modified"
        ).grid(column=0, row=3, sticky="w", padx=(20, 0))

        # NEW: Category organization section
        org_frame = ttk.LabelFrame(options_frame, text="File Organization", padding=10)
        org_frame.grid(column=0, row=8, columnspan=2, sticky="ew", pady=(10, 0))

        self.organize_by_category_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(
            org_frame,
            text="Organize files into category folders (requires AI categorization)",
            variable=self.organize_by_category_var,
        ).grid(column=0, row=0, sticky="w")

        ttk.Label(org_frame, text="Organize into:").grid(column=0, row=1, sticky="w", pady=(6, 0))
        self.organize_location_var = tk.StringVar(value="subdirs")
        ttk.Radiobutton(
            org_frame,
            text="Create subfolders in scanned directory",
            variable=self.organize_location_var,
            value="subdirs"
        ).grid(column=0, row=2, sticky="w", padx=(20, 0))
        ttk.Radiobutton(
            org_frame,
            text="Specify different target directory",
            variable=self.organize_location_var,
            value="custom"
        ).grid(column=0, row=3, sticky="w", padx=(20, 0))

        self.organize_target_var = tk.StringVar()
        self.organize_target_entry = ttk.Entry(org_frame, textvariable=self.organize_target_var, state="disabled")
        self.organize_target_entry.grid(column=0, row=4, sticky="ew", padx=(40, 6), pady=(4, 0))
        self.organize_browse_button = ttk.Button(org_frame, text="Browse", command=self._browse_organize_target, state="disabled")
        self.organize_browse_button.grid(column=1, row=4, pady=(4, 0))

        # Bind the radio button to enable/disable custom target controls
        self.organize_location_var.trace_add("write", self._on_organize_location_changed)

        # NEW: Safety options section
        safety_frame = ttk.LabelFrame(options_frame, text="Safety Options", padding=10)
        safety_frame.grid(column=0, row=9, columnspan=2, sticky="ew", pady=(10, 0))

        self.skip_symlinks_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            safety_frame,
            text="Skip symbolic links and shortcuts",
        ...
        ttk.Checkbutton(
            safety_frame,
            text="Create operation log for rollback capability",
        ...
        # NEW: Apply File Operations button
        self.apply_operations_button = ttk.Button(
            summary_frame,
            text="Apply File Operations...",
            command=self._on_apply_operations_clicked,
            state="disabled"
        )
        self.apply_operations_button.grid(column=2, row=0, sticky="w", padx=(8, 0))
    ...
    def _browse_organize_target(self) -> None:
        directory = filedialog.askdirectory()
        if directory:
            self.organize_target_var.set(directory)

    def _on_organize_location_changed(self, *args):
        """Enable/disable custom target controls based on selection."""
        if self.organize_location_var.get() == "custom":
            self.organize_target_entry.configure(state="normal")
            self.organize_browse_button.configure(state="normal")
        else:
            self.organize_target_entry.configure(state="disabled")
            self.organize_browse_button.configure(state="disabled")
    ...
    def _handle_results(self, results: ScanResults) -> None:
        ...
        # Enable Apply Operations button if we have results
        self.apply_operations_button.configure(state="normal")
    ...
    def _on_apply_operations_clicked(self):
        """Handler for Apply File Operations button."""
        if not self.current_results:
            messagebox.showwarning("No results", "Run a scan first.")
            return

        # Validate options
        if not self.handle_duplicates_var.get() and not self.organize_by_category_var.get():
            messagebox.showinfo(
        ...
        if self.organize_by_category_var.get() and not self.classifier_var.get():
            messagebox.showerror(
                "AI categorization required",
                "File organization by category requires AI categorization to be enabled and a scan to be completed with categories assigned."
            )
            return

        # Prepare options dict
        options = {
            "primary_mode": self.primary_selection_var.get(),
            "create_log": self.create_backup_var.get(),
            "log_path": str(self.current_results.root / "duperanger_operations.json"),
            "skip_symlinks": self.skip_symlinks_var.get(),
        }

        if self.organize_by_category_var.get():
            if self.organize_location_var.get() == "custom":
                target_dir = self.organize_target_var.get().strip()
                if not target_dir or not Path(target_dir).exists():
                    messagebox.showerror("Invalid target", "Please specify a valid target directory for organization.")
                    return
                options["organize_base_dir"] = target_dir
            else:
                options["organize_base_dir"] = str(self.current_results.root)

        # Create operation manager
        self.operation_manager = FileOperationManager(self.current_results, options)

        # Plan operations
        operations = []
        try:
            if self.handle_duplicates_var.get():
                dup_ops = self.operation_manager.plan_duplicate_operations()
                operations.extend(dup_ops)

            if self.organize_by_category_var.get():
                cat_ops = self.operation_manager.plan_category_operations()
                operations.extend(cat_ops)
        except Exception as e:
            messagebox.showerror("Planning failed", f"Failed to plan operations: {e}")
            return

        if not operations:
            messagebox.showinfo("No operations needed", "No file operations are needed based on current settings.")
            return

        # Show preview dialog
        preview = OperationPreviewDialog(self.root, operations)
        result = preview.show()
        ...
        self._execute_operations_async(operations)

    def _execute_operations_async(self, operations: list[FileOperation]):
        """Execute operations in background thread with progress updates."""

        # Disable UI
        self._set_ui_state(scanning=True)
        self.apply_operations_button.configure(state="disabled")
        self.progress_var.set("Applying file operations...")

        # Create progress dialog
        progress_dialog = tk.Toplevel(self.root)
        progress_dialog.title("Applying Operations")
        progress_dialog.geometry("600x150")
        ...
        progress_bar = ttk.Progressbar(progress_dialog, mode="determinate", maximum=len(operations))
        ...
        def progress_callback(current: int, total: int, op: FileOperation):
            progress_var.set(f"Processing {current + 1} of {total}: {op.source_path.name}")
        ...
        def worker():
            self.stop_event.clear()
            success, failed, skipped = self.operation_manager.execute_operations(
                operations,
                self.stop_event,
                progress_callback
            )
            ...
            self.root.after(0, lambda: self._operations_complete(
                progress_dialog,
                success,
                failed,
                skipped,
    ...
    def _operations_complete(self, progress_dialog, success: int, failed: int, skipped: int, total: int):
        ...
        if failed > 0:
            summary += "\n\nCheck the operation log for details on failed operations."

        messagebox.showinfo("Operations Complete", summary)

        # Suggest re-scan
        if success > 0:
            if messagebox.askyesno(
                "Re-scan?",
                "Would you like to re-scan the directory to see the updated file organization?"
            ):
